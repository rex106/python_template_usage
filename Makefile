...
fmt:              ## Format code using black & isort.
	$(ENV_PREFIX)isort python_template_usage/
	$(ENV_PREFIX)black -l 79 python_template_usage/
	$(ENV_PREFIX)black -l 79 tests/
	$(ENV_PREFIX)black -l 79 .github/templates/

.PHONY: lint
lint:             ## Run pep8, black, mypy linters.
	$(ENV_PREFIX)flake8 python_template_usage/
	$(ENV_PREFIX)flake8 .github/templates/
	$(ENV_PREFIX)black -l 79 --check python_template_usage/
	$(ENV_PREFIX)black -l 79 --check tests/
	$(ENV_PREFIX)black -l 79 --check .github/templates/
	$(ENV_PREFIX)mypy --ignore-missing-imports python_template_usage/

.PHONY: test
test: lint        ## Run tests and generate coverage report.
	$(ENV_PREFIX)pytest -v --cov-config .coveragerc --cov=python_template_usage -l --tb=short --maxfail=1 tests/
...
release:          ## Create a new tag for release.
	...
	@git tag $${TAG}
	@echo "$${TAG}" > python_template_usage/VERSION
	@$(ENV_PREFIX)gitchangelog > HISTORY.md
	@git add python_template_usage/VERSION HISTORY.md
...
switch-to-poetry: ## Switch to poetry package manager.
	...
	@rm -rf .venv
	@poetry init --name=python_template_usage --author=rex106
